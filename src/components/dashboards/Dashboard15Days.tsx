...
export default function Dashboard15Days() {
  ...
  const calculateStats = (): DashboardStats => {
    ...
    const totalResponsibilities = filteredResponses.reduce((sum, r) => sum + parseInt(r.responses.responsibilities || '0'), 0)
    const totalSupport = filteredResponses.reduce((sum, r) => sum + parseInt(r.responses.support || '0'), 0)
    const totalIntegration = filteredResponses.reduce((sum, r) => sum + parseInt(r.responses.integration || '0'), 0)
    const totalProcesses = filteredResponses.reduce((sum, r) => sum + parseInt(r.responses.processes || '0'), 0)
    ...
    return {
      ...
      averageResponsibilities: totalResponsibilities / filteredResponses.length,
      averageSupport: totalSupport / filteredResponses.length,
      averageIntegration: totalIntegration / filteredResponses.length,
      averageProcesses: totalProcesses / filteredResponses.length,
      responsesByOffice,
      responsesByPosition
    }
  }
  ...
  return (
    <div className="space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-2xl font-bold">
              {((stats.averageResponsibilities + stats.averageSupport + stats.averageIntegration + stats.averageProcesses) / 4).toFixed(1)}/5
            </div>
            <Progress 
              value={((stats.averageResponsibilities + stats.averageSupport + stats.averageIntegration + stats.averageProcesses) / 4) * 20} 
              className="mt-2" 
            />
          </CardContent>
        </Card>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <Card>
          <CardHeader>
            <CardTitle>Métricas de Adaptação</CardTitle>
            <CardDescription>Avaliação dos principais aspectos de integração</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-4">
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm">Responsabilidades</span>
                  <span className="text-sm font-medium">{stats.averageResponsibilities.toFixed(1)}/5</span>
                </div>
                <Progress value={(stats.averageResponsibilities / 5) * 100} />
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm">Apoio Recebido</span>
                  <span className="text-sm font-medium">{stats.averageSupport.toFixed(1)}/5</span>
                </div>
                <Progress value={(stats.averageSupport / 5) * 100} />
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm">Integração</span>
                  <span className="text-sm font-medium">{stats.averageIntegration.toFixed(1)}/5</span>
                </div>
                <Progress value={(stats.averageIntegration / 5) * 100} />
              </div>
              <div>
                <div className="flex justify-between mb-1">
                  <span className="text-sm">Processos Claros</span>
                  <span className="text-sm font-medium">{stats.averageProcesses.toFixed(1)}/5</span>
                </div>
                <Progress value={(stats.averageProcesses / 5) * 100} />
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Distribuição por Sede</CardTitle>
            <CardDescription>Respostas por localização</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              {Object.entries(stats.responsesByOffice).map(([office, count]) => (
                <div key={office} className="flex items-center justify-between">
                  <span className="capitalize">{office.replace('sede', 'Sede ')}</span>
